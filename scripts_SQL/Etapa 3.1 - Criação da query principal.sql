---Etapa 3 / Passo 1: Criação da query principal combinando todas as trasferências (entrada, saída e pix) em uma única tabela -> total_trasfers

--Informações necessárias:

--transaction_id
--customer_id
--full_name = junção da coluna primeiro e último nome
--account_id
--amount
--status = trazer somente completed
--type_transaction = se é transferência de saída, entrada ou pix entrada e saída
--date_completed = data que finalizou a movimentação com status completed
--month = mês que foi realizada a transação, considerando jan/20 a dez/20
